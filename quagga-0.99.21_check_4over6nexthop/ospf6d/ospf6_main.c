...
static void update_neighbor_period (struct in6_addr *ipv6_addr)
{
    ...
    for (ifa = ifList; ifa != NULL; ifa = ifa->ifa_next)
    {
        ...
        if (j == INTERFACE_NUM)
        {
            ...
            {
#ifdef OSPF6_DEBUG
                zlog_debug ("find the neighbor on the interface:%s\n", ifa->ifa_name);
#endif
...
static int process_statellite_message (char *recv_buf)
{
    ...
    struct comm_head_tmp *commhead = (struct comm_head_tmp *) recv_buf;

    total_msg_len = ntohs (commhead->len) - sizeof (struct comm_head_tmp);
...
static inline void recv_local (void)
{
    ...
    while (1)
    {
        ...
        {
            printf("sizeof(statellite_period)=%lu\n",sizeof(struct statellite_period));
            //commhead=(struct comm_head_tmp *)info;
