    ...
    if (oi->reference_count > 0)
    {
        ...
        if (lsa_bak == NULL)
        {
            ...
            if (ospf6_check_lsa_maxage_prefix (oi, lsa) == 0)
            {
                ...
                {
#ifdef OSPF6_DEBUG
                    zlog_debug ("[%d]%s() The interface %s has %d neighbor in leaving, then install lsas to new lsas list, type:%u", __LINE__, __func__, oi->interface->name, oi->reference_count, lsatype);
#endif
        ...
        {
#ifdef OSPF6_DEBUG
            zlog_debug ("[%d]%s() in area->lsdb_bak find lsa is:%s", __LINE__, __func__, lsa_bak->name);
#endif
        }
    }
#endif
