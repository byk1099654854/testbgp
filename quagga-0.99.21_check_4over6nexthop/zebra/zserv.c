...
static void openflow_event (enum event event, int sock, struct zserv *client);
...
static void
openflow_client_create (int sock)
{
    struct zserv *client;
    ...
    /* Make new read thread. */
    openflow_event (ZEBRA_READ, sock, client);
}
...
static int
zebra_server_read (struct thread *thread)
{
    int sock;
    struct zserv *client;
    size_t already;
    ...
    if(ret>0)
    {
        memcpy(client->ibuf->data+client->ibuf->size,recvbuf,ret);
        client->ibuf->size += ret;
        //printf(" ibif_len = %d\n",client->ibuf->size);
        //printf("endp = %d\n",client->ibuf->endp);
        if(client->ibuf->endp == 0)
        {
            head = (struct comm_head *)recvbuf;
            client->ibuf->endp = ntohl(head->len);
        }


        if(client->ibuf->size ==  client->ibuf->endp)
        {
            memcpy(recvbuf,client->ibuf->data,client->ibuf->size);
            zebra_server_process_msg(recvbuf,client->ibuf->size);
        }
...
static int
zebra_server_accept (struct thread *thread)
{
    ...
    /* Reregister myself. */
    openflow_event (ZEBRA_SERV, accept_sock, NULL);
    ...
    /* Create new zebra client. */
    openflow_client_create (client_sock);
...
openflow_serv ()
{
    ...
    if ((accept_sock = socket(PF_INET6, SOCK_STREAM, 0)) == -1)
    {
    ...
    // addr.sin_addr.s_addr = inet_addr ("188.0.0.101");
#endif


    bzero(&my_addr, sizeof(my_addr));
    my_addr.sin6_family = PF_INET6;    // IPv6
    my_addr.sin6_port = htons(12345);   // IPv6
    ...
    ret =bind(accept_sock, (struct sockaddr *) &my_addr, sizeof(struct sockaddr_in6));
...
static void
openflow_event (enum event event, int sock, struct zserv *client)
{
    switch (event)
    {
    case ZEBRA_SERV:
            thread_add_read (zebrad.master, zebra_server_accept, client, sock);
        break;
    case ZEBRA_READ:
        client->t_read =
            thread_add_read (zebrad.master, zebra_server_read, client, sock);
        break;
    case ZEBRA_WRITE:
        /**/
        break;
...
void
openflow_zserv_socket_init (char *path)
{
