...
//	printf("-----a1 ifp->flags & IFF_RUNNING = %d \n",ifp->flags & IFF_RUNNING);
    ifreq_set_name (&ifreq, ifp);
//	printf("-----a2 ifp->flags & IFF_RUNNING = %d \n",ifp->flags & IFF_RUNNING);

    ret = if_ioctl (SIOCGIFFLAGS, (caddr_t) &ifreq);
//	printf("-----a3 ifp->flags & IFF_RUNNING = %d \n",ifp->flags & IFF_RUNNING);
    if (ret < 0)
    {
     ...
     */
//	printf("-----c1 ifp->flags & IFF_RUNNING = %d \n",ifp->flags & IFF_RUNNING);
    SET_FLAG(ifreq.ifr_flags, IFF_RUNNING);
//	printf("-----c2 ifp->flags & IFF_RUNNING = %d \n",ifp->flags & IFF_RUNNING);
...
//	printf("-----t1 ifp->flags & IFF_RUNNING = %d \n",ifp->flags & IFF_RUNNING);
    if(ifp->flags & IFF_RUNNING) // add wjh for link-det
        if_flags_update (ifp, (ifreq.ifr_flags & 0x0000ffff));
//	printf("-----t2 ifp->flags & IFF_RUNNING = %d \n",ifp->flags & IFF_RUNNING);
}
...
{
//printf("in flie:%s\t func:%s\t line:%d\n",__FILE__,__func__,__LINE__);
    int ret;
    struct ifreq ifreq;
...
if_prefix_delete_ipv6 (struct interface *ifp, struct connected *ifc)
{
    ...
    {
        printf("this is  fe80 address, return here\n");
        return;
    }
    memset (&ifreq, 0, sizeof (struct in6_ifreq));
    ...
    char pfx_buf[120];
    memset(pfx_buf,0,120);
    prefix2str (ifc->address, pfx_buf, sizeof (pfx_buf));
    printf("del kernel ipv6 address %s %s\n",ifp->name,pfx_buf);
