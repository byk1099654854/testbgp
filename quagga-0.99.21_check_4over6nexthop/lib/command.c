...
#include "bgp_linklist.h"


struct sectionengine_table *sectionengine_table_head;
struct processing_engine_table *processingengine_table_head;
...
DEFUN (show_version,
       ...
       "show version",
       SHOW_STR
       "Displays NetWire4604 version\n")
{
    struct sysinfo s_info;
    long uptime = 0;

    vty_out (vty, "NetWire4604 (%s).%s", "1.0.20131204", host.name?host.name:"\r\n",
             VTY_NEWLINE);
...
/* Write current configuration into file. */ //added by wjh for dpdk.conf
void write_file_for_dpdk_conf(struct vty *vty)
{

    char *dpdk_conf = "/usr/local/etc/dpdk.conf";
#if 1
    unsigned int i;
    int fd;
    ...
    config_file = dpdk_conf;

//	 vty_out (vty, "configuration file %s.%s", config_file,VTY_NEWLINE);
...
#if 0
    if(strcmp(config_file,"/usr/local/etc/dpdk.conf")==0)
    {
        vty_out (vty, "Configuration saved to dpdk.%s%s", "config",
                 VTY_NEWLINE);
    }
#endif
    /*
...
{
#if 1
    system("/bin/cp /usr/local/etc/dpdk.conf.sav /usr/local/etc/zebra.conf.sav");
    system("/bin/cp /usr/local/etc/dpdk.conf /usr/local/etc/zebra.conf");
#endif
...
void showsectionenginetable(struct vty *vty)
{
    int i =0 ;
    struct sectionengine_table *pos = NULL;
    vty_out (vty, "! sectionengine table %s", VTY_NEWLINE);
    char buf[BUFSIZ];

    if(sectionengine_table_head == NULL)
    {

        vty_out (vty, "section-engine is NULL %s",VTY_NEWLINE);
    }
    else
    {

        vty_out (vty, "section-engine %s %s ",sectionengine_table_head->sectionengine.sectionenginename,VTY_NEWLINE);
    ...
    {
        vty_out (vty, "section-engine %s",pos->sectionengine.sectionenginename);
        for(i =0; i<pos->sectionengine.prefixnum; i++)
        {

            prefix2str(&pos->sectionengine.v6prefix[i],buf,BUFSIZ);
            vty_out (vty, "	%s", pos->sectionengine.isused,buf);
        }
        vty_out (vty, " isbind %d %s", VTY_NEWLINE);
    }
    vty_out (vty, "! sectionengine table end%s", VTY_NEWLINE);

}

void showprocessingenginetable(struct vty *vty)
{
    int i =0 ;
    struct processing_engine_table *pos = NULL;
    vty_out (vty, "! processingengine table %s", VTY_NEWLINE);
    char type[8];

    for(pos = processingengine_table_head; pos!= NULL; pos = pos->next)
    {

        if(pos->processingengine.type == 1)
        {
            strcpy(type,"ivi");
        }
        else if(pos->processingengine.type == 2)
        {
            strcpy(type,"nat64");
        }
        else if(pos->processingengine.type == 3)
        {
            strcpy(type,"4over6");
        }
        else if(pos->processingengine.type == 4)
        {
            strcpy(type,"FW");
        }
        ...
        vty_out (vty, "processing-engine %s %d %d %s  %s isbind %d",pos->processingengine.processingenginename,pos->processingengine.memoryvalue,pos->processingengine.bandwidthvalue,type,pos->processingengine.rt_name,pos->processingengine.isused);

        vty_out (vty, " %s", VTY_NEWLINE);
    }
    vty_out (vty, "! processingengine table end%s", VTY_NEWLINE);

}



void showflowenginetable(struct vty *vty)
{
    //int i =0 ;
    struct flow_engine_table *pos = NULL;
    vty_out (vty, "! flowengine table %s", VTY_NEWLINE);

    for(pos = flowengine_table_head; pos!= NULL; pos = pos->next)
    {


        vty_out (vty, "flow-engine %s %s %s ",pos->flowengine.flowenginename,pos->flowengine.sectionenginename->sectionengine.sectionenginename,pos->flowengine.processingenginename->processingengine.processingenginename);

        vty_out (vty, " %s", VTY_NEWLINE);
    }
    vty_out (vty, "! flowengine table end%s", VTY_NEWLINE);
