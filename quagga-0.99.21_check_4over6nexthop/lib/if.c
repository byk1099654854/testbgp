...
extern struct cmd_element no_ipv6_address_cmd;
       ...
       "Interface's name\n")
{
    ...
    if(ifp == NULL)
    {
        vty_out (vty, "%% No such interface 2%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
...
int get_ifindex_by_ifname(char *ifname)
{
    int a=0,b=0;
    int ret;
    sscanf(ifname,"vEth%d_%d",&a,&b);
    //ret = a*5 + b;
    ret = (a-1)*5 + b;//interface name start from vEth1_0
    return ret;
...
{
    // printf("------------send_interface_up_down_to_dpdk------------\n");

    //vty_out (vty, "%% in send_interface_up_down_to_dpdk %s  status %d %s",ifname,status, VTY_NEWLINE);
    ...
    {
        comm = (struct comm_head *) malloc (sizeof (struct comm_head) + 2);
        if (comm == NULL)
        {
            printf ("%s\n", "flow engine info head malloc failed");
            return -1;
        }
        memset (comm, 0, sizeof (struct comm_head) + 2);

        comm->type = 0x35;
        comm->len = htonl(sizeof (struct comm_head) + 2);
        comm->data[0] = get_ifindex_by_ifname(ifname);
        // printf("in send shutdown ifname = %s\t index = %d\n",ifname,comm->data[0]);
        comm->data[1] = status;

        sockfd = connect_dpdk();
        ret = send (sockfd, (char *) comm, sizeof (struct comm_head) + 2, 0);
        if (ret < 0)
        {
            printf ("%s\n", "send comm failed");
            close (sockfd);
    ...
    if(status == 1)
    {
        //sprintf(sys_cmd,"ifconfig %s up",ifname);
        //system(sys_cmd);
#if 0
        sprintf(sys_cmd,"/bitway/test/mp_server/config_eth_mac_and_misc.sh %d",get_ifindex_by_ifname(ifname));
        system(sys_cmd);
#endif
    }
    if(status == 0)
    {
        //sprintf(sys_cmd,"ifconfig %s down",ifname);
        //system(sys_cmd);
    }
    return 0;
...
DEFUN (interfacegigabitethernet,
       interfacegigabitethernet_cmd,
       ...
       "Interface's name\n")
{
    ...
    if(ifp == NULL)
    {
        vty_out (vty, "%% No such interface 1%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
    ...
    //send_interface_up_down_to_dpdk(vty,argv[0],1);
    return CMD_SUCCESS;


}
/*
DEFUN_NOSH (no_interfacegigabitethernet,
            no_interfacegigabitethernet_cmd,
            ...
            "Interface's name\n")
{
	...
	memset(name,0,20);
	sscanf(argv[0],"%d_%d",&i,&j);
	sprintf(name,"vEth%d_%d",i,j);
	memcpy(argv[0],name,20);

	if(i>=4||i<0)
	{
        vty_out (vty, "%% phynum must is [0,3]%s", name, VTY_NEWLINE);
        return CMD_WARNING;
...
/*
	 for (ALL_LIST_ELEMENTS_RO (ifp->connected, addrnode, ifc))
     {
            if (CHECK_FLAG (ifc->conf, ZEBRA_IFC_CONFIGURED))
            {
                ...
                p = ifc->address;
                vty_out (vty, "no ip%s address %s/%d",
                         p->family == AF_INET ? "" : "v6",
                         inet_ntop (p->family, &p->u.prefix, buf, sizeof(buf)),p->prefixlen);
