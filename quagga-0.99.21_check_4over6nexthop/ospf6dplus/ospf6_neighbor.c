...
#ifdef OSPF6_DEBUG
    //sangmeng add debug
    zlog_debug("[%d]%s thread off connection_period_timer, single_hello_timer, circle_period_timer", __LINE__, __func__);
#endif
    //sangmeng add
    THREAD_OFF (on->connection_period_timer);
    THREAD_OFF (on->single_hello_timer);
...
static float get_seconds_has_gone_today(void)
{
    ...
    printf(" %.f secondes has gone today\n", seconds);
...
hello_received (struct thread *thread)
{
...
#endif
    if ((on->state == OSPF6_NEIGHBOR_LEAVING) || (on->single_hello_timer))
    {
        on->connection_period_timer_state = 0;
    }
#ifdef OSPF6_DEBUG
    zlog_debug ("on->connection_period_timer_state:%d, ifname:%s, neighbor ipv6 address:%s", on->connection_period_timer_state, on->ospf6_if->interface->name, ipv6_string);
#endif
    ...
    /*b) judge the single hello timer is exist */
    if (on->single_hello_timer)
    {
#ifdef OSPF6_DEBUG
        zlog_debug("recv hello packet, del single hello timer, neighbor ipv6 address:%s", ipv6_string);
#endif
        /*del single hello timer */
        THREAD_OFF (on->single_hello_timer);
...
exchange_done (struct thread *thread)
{
    ...
    */
    ...
    if (on->leaving_exchange_state == 1)
    {
        ...
        {
#ifdef OSPF6_DEBUG
            zlog_debug("[%d]%s() hi*********, lsa->name:%s", __LINE__, __func__, lsa->name);
#endif
...
#ifdef OSPF6_DEBUG
    zlog_debug("[%d]%s() ospf6 neighbor state change to FULL, neighbor ipv6 address:%s", __LINE__, __func__, ipv6_string);
#endif
    ospf6_neighbor_state_change (OSPF6_NEIGHBOR_FULL, on);
...
#endif
#ifdef OSPF6_DEBUG
        zlog_debug("[%d]%s() ospf6 neighbor state must be Leaving, now is:%s", __LINE__, __func__, ospf6_neighbor_state_str[on->state]);
#endif
...
int connection_period_timer (struct thread *thread)
{
    ...
    on->single_hello_timer = thread_add_timer (master, single_hello_timer, on, on->ospf6_if->hello_interval);
#endif
...
//sangmeng add for OSPF+ 16.4.21
int single_hello_timer (struct thread *thread)
{
    ...
    on->single_hello_timer = NULL;
    /*change the neighbor to Leaving */
    zlog_debug("Single hello timer is active, change neighbor state to LEAVING, ifname:%s", on->ospf6_if->interface->name);
    ospf6_neighbor_state_change (OSPF6_NEIGHBOR_LEAVING, on);
