...
static void ospf6_send (struct in6_addr *src, struct in6_addr *dst, struct ospf6_interface *oi, struct ospf6_header *oh)
{
    ...
    /* send message */
    printf("oi->interface->ifindex = %d\n",oi->interface->ifindex);
    if(oi->interface->ifindex == 0)
    {
        zlog_err ("link down Could not send entire message");
        return ;
    }
...
static int ospf6_examine_lsa_prefix (struct ospf6_prefix *current,  unsigned length, const u_int32_t req_num_pfxs, struct prefix_ipv6 *p)
{
    ...
    while (length)
    {
        ...
        memcpy (&in6, OSPF6_PREFIX_BODY (current), OSPF6_PREFIX_SPACE (current->prefix_length));
        inet_ntop (AF_INET6, &in6, buf, sizeof (buf));
        printf ("[%d]%s() lsa header type is OSPF6_LSTYPE_INTRA_PREFIX, prefix:%s/%d\n", __LINE__, __func__, buf, current->prefix_length);

        if (p->prefixlen == current->prefix_length)
        {
            if (ipv6_prefix_equal (&p->prefix, &in6, p->prefixlen) == 1)
            {
                printf ("[%d]%s() the address is same, return 1\n", __LINE__, __func__);
                return 1;
            }
        }
...
static int ospf6_get_lsa_header_type (struct ospf6_lsa_header *lsah, struct prefix_ipv6 *p)
{
    ...
    case OSPF6_LSTYPE_INTRA_PREFIX:
        ...
        zlog_debug ("[%d]%s()ret============%d", __LINE__, __func__, ret);
        if (ret == 1)
            return 1;
#endif
#ifdef OSPF6_DEBUG
...
static void get_oi_v6_address (struct ospf6_interface *oi, struct prefix_ipv6 *ipv6)
{
    ...
    for (ALL_LIST_ELEMENTS_RO (ifp->connected, node, connected))
    {
        if ((connected->address->family == AF_INET6))
        {
            ...
            printf ("[%d]%s()the interface:%s address is:%s/%d\n", __LINE__, __func__, ifp->name, buf, p->prefixlen);
...
int ospf6_check_lsa_maxage_prefix (struct ospf6_interface *oi, struct ospf6_lsa *lsa)
{
    ...
    //if (OSPF6_LSA_IS_MAXAGE (lsa))
    {
        ...
        zlog_debug ("[%d]%s()>>>>>>>>>maxage lsa:%s, then check prefix, ret:%d", __LINE__, __func__, lsa->name, ret);
        return ret;
    }
